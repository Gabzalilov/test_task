...
При запуске `web`-контейнер сначала применяет миграции Alembic, потом стартует FastAPI-приложение. Значения в `.env` можно менять под свое окружение; дефолтные значения в `.env.example` совпадают с Docker Compose-стеком.
...
5. С вероятностью `0.15` имитируется сбой внешнего сервиса.
6. При успехе статус меняется на `confirmed`, после этого логируется mock-уведомление.
7. При временном сбое задача уходит на retry с backoff.
8. Если попытки закончились, бронь переводится в `failed`.
